...
old=`mktemp` && new=`mktemp` || err 'temp file cannot be created'
ls | tee $new | sed "$escape" > $old
$editor $new
[ `cat $old | wc -l` != `cat $new | wc -l` ] && err 'line count differs'
cat $new | sed "$escape" | paste $old - | xargs -n 2 sh -c "$movecmd" "$cmd"
rm $old $new